...
UndoInfo MoveMaker::make_move(const model::Move& move)
{
    ...
    if (move.is_any_castle()) {
        ...
        auto king_type = pos_.is_w ? PieceType::W_KING
                                   : PieceType::B_KING;
    ...
    PieceType moved_piece_type = remove_moved_piece_from_board(from_sq);
    ...
    if (move.is_any_capture()) {
        ...
        PieceType captured_piece_type = pos_.bbs.get_piece_type_at(capture_sq, !pos_.is_w);
...
void MoveMaker::update_castle_rights(const model::Move& move, UndoInfo& undo_info) 
{
    ...
    if (pos_.is_w) {
        ...
        if (undo_info.moved_piece_type == PieceType::W_KING) {
            pos_.c_rights &= ~masks::W_BOTH_SIDES_CASTLE_RIGHTS_MASK;
        ...
        bool rook_moved = (move.from() == constants::W_KSIDE_ROOK_START_SQ 
                        ...
                        && undo_info.moved_piece_type == PieceType::W_ROOK;

        bool opp_rook_captured = move.is_any_capture() 
                              && undo_info.captured_piece_type == PieceType::B_ROOK
        ...
        if (undo_info.moved_piece_type == PieceType::B_KING) {
            pos_.c_rights &= ~masks::B_BOTH_SIDES_CASTLE_RIGHTS_MASK;
            z_hasher_.update_castle_rights(old_castle_rights, pos_.c_rights);
            return;
        }
        ...
        bool rook_moved = (move.from() == constants::B_KSIDE_ROOK_START_SQ 
                        ...
                        && undo_info.moved_piece_type == PieceType::B_ROOK;

        bool opp_rook_captured = move.is_any_capture() 
                              && undo_info.captured_piece_type == PieceType::W_ROOK
...
void MoveMaker::make_castle_move(bool is_kside)
{
    ...
    if (pos_.is_w) {
        ...
        pos_.bbs.clr_bit_at(king_from_sq, PieceType::W_KING);
        pos_.bbs.set_bit_at(king_to_sq, PieceType::W_KING);
        pos_.bbs.clr_bit_at(rook_from_sq, PieceType::W_ROOK);
        pos_.bbs.set_bit_at(rook_to_sq, PieceType::W_ROOK);

        z_hasher_.xor_piece_type_at(king_from_sq, PieceType::W_KING);
        z_hasher_.xor_piece_type_at(king_to_sq,   PieceType::W_KING);
        z_hasher_.xor_piece_type_at(rook_from_sq, PieceType::W_ROOK);
        z_hasher_.xor_piece_type_at(rook_to_sq,   PieceType::W_ROOK);
    } else {
        ...
        pos_.bbs.clr_bit_at(king_from_sq, PieceType::B_KING);
        pos_.bbs.set_bit_at(king_to_sq, PieceType::B_KING);
        pos_.bbs.clr_bit_at(rook_from_sq, PieceType::B_ROOK);
        pos_.bbs.set_bit_at(rook_to_sq, PieceType::B_ROOK);

        z_hasher_.xor_piece_type_at(king_from_sq, PieceType::B_KING);
        z_hasher_.xor_piece_type_at(king_to_sq,   PieceType::B_KING);
        z_hasher_.xor_piece_type_at(rook_from_sq, PieceType::B_ROOK);
        z_hasher_.xor_piece_type_at(rook_to_sq,   PieceType::B_ROOK);
    }
}

PieceType MoveMaker::remove_moved_piece_from_board(sq_t from_sq) 
{
    // Determine the piece type of the piece being moved
    PieceType  moved_piece_type = pos_.bbs.get_piece_type_at(from_sq, pos_.is_w);

    // Update zobrist hash
    z_hasher_.xor_piece_type_at(from_sq, moved_piece_type);

    // Clear the piece from bbs, squarelookup and gamestate bitmasks
    pos_.bbs.clr_bit_at(from_sq, moved_piece_type);
...
{
    pos_.bbs.set_bit_at(to_sq, moved_piece_type);
    z_hasher_.xor_piece_type_at(to_sq, moved_piece_type);
}

void MoveMaker::remove_captured_piece_from_board(sq_t capture_sq, PieceType captured_piece_type)
{
    // Remove captured piece from models
    pos_.bbs.clr_bit_at(capture_sq, captured_piece_type);
