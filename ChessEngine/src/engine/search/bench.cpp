...
// Note: This was taken from https://github.com/official-stockfish/Stockfish/blob/master/src/benchmark.cpp
static const std::vector<std::string> bench_set {
    ...
    // Todo: Add back in after Static Exchange Evaluation implementation
...
static constexpr int BENCH_FIXED_DEPTH = 6; 
...
void Searcher::bench()
{
    ...
    for (const auto& fen : bench_set) {
        ...
        uci::GoParams go_params{};
        go_params.depth = BENCH_FIXED_DEPTH;
        TimeManager tm{go_params, true};
        search(tm);
    ...
    int nodes_per_s = node_count / elapsed_seconds.count();
    std::cout << "Nodes per second: " << nodes_per_s << std::endl;
    std::cout << "Benchmark: " << node_count << " total nodes." << std::endl;
}
