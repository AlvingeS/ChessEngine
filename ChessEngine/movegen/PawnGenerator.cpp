...
namespace movegen {
    PawnGenerator::PawnGenerator(board::BitBoards& bitboards, board::GameStateBitMasks& gameStateBitmasks, int& moveIndex, CommonLogic* commonLogic) 
    ...
    void PawnGenerator::generate(bool isWhite, std::vector<move::Move>& moveList, int currentDepth, perft::SearchMemory& searchMemory) {
        ...
        for (int currentPawnIndex : _pawnIndices) {
            ...
            if (_freeMovesIndices.size() == 2) {
                ...
                _commonLogic->addMove(currentPawnIndex, _freeMovesIndices[singleStepIndex], (move::Move::QUITE_FLAG), moveList, _moveIndex);
                _commonLogic->addMove(currentPawnIndex, _freeMovesIndices[doubleStepIndex], (move::Move::DOUBLE_PAWN_PUSH_FLAG), moveList, _moveIndex);
                ...
                if (canPromote) {
                    _commonLogic->addMove(currentPawnIndex, _freeMovesIndices[0], move::Move::KNIGHT_PROMO_FLAG, moveList, _moveIndex);
                    _commonLogic->addMove(currentPawnIndex, _freeMovesIndices[0], move::Move::BISHOP_PROMO_FLAG, moveList, _moveIndex);
                    _commonLogic->addMove(currentPawnIndex, _freeMovesIndices[0], move::Move::ROOK_PROMO_FLAG, moveList, _moveIndex);
                    _commonLogic->addMove(currentPawnIndex, _freeMovesIndices[0], move::Move::QUEEN_PROMO_FLAG, moveList, _moveIndex);
                } else {
