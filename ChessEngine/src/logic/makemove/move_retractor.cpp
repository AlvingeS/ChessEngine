...
void MoveRetractor::unmake_castle_move(bool was_kside)
{
    ...
    if (was_white) {
        ...
        pos_.bbs.clr_bit_at(king_to_sq, PieceType::W_KING);
        pos_.bbs.set_bit_at(king_from_sq, PieceType::W_KING);
        pos_.bbs.clr_bit_at(rook_to_sq, PieceType::W_ROOK);
        pos_.bbs.set_bit_at(rook_from_sq, PieceType::W_ROOK);
    } else {
        ...
        pos_.bbs.clr_bit_at(king_to_sq, PieceType::B_KING);
        pos_.bbs.set_bit_at(king_from_sq, PieceType::B_KING);
        pos_.bbs.clr_bit_at(rook_to_sq, PieceType::B_ROOK);
        pos_.bbs.set_bit_at(rook_from_sq, PieceType::B_ROOK);
    }
}
...
void MoveRetractor::remove_previously_moved_piece_from_board(
    ...
    sq_t to_sq,
    PieceType previously_moved_piece_type) 
{
    bool was_white = !pos_.is_w;
    ...
    if (not move.is_any_promo()) {
        pos_.bbs.clr_bit_at(to_sq, previously_moved_piece_type);
    } else {
        PieceType promo_piece_type = utils::get_promotion_piece_type(move.flag(), was_white);
        pos_.bbs.clr_bit_at(to_sq, promo_piece_type);
    }
}
...
PieceType MoveRetractor::determine_moved_piece_type(
    const model::Move& move, 
    sq_t to_sq) const
{
    bool was_white = !pos_.is_w;
    ...
    previously_moved_piece_type = move.is_any_promo() ? (was_white ? PieceType::W_PAWN
                                                                   : PieceType::B_PAWN)
                                                      : pos_.bbs.get_piece_type_at(to_sq, was_white);
    ...
    logic::UndoInfo& undo_info) 
{
    ...
    PieceType  previously_moved_piece_type = determine_moved_piece_type(previous_move, to_sq);
