...
namespace move {
    BitBoardUpdater::BitBoardUpdater(board::BitBoards& bitboards) : _bitboards(bitboards) {}

    void BitBoardUpdater::makeCastleMove(bool isWhite, bool isKingSide) {
            if (isWhite) {
                if (isKingSide) {
    ...
    void BitBoardUpdater::unmakeCastleMove(bool wasWhite, bool wasKingSide) {
        if (wasWhite) {
            if (wasKingSide) {
                _bitboards.getWhiteKingBitboard() &= ~(1ULL << 1);
                _bitboards.getWhiteKingBitboard() |= (1ULL << 3);
    ...
    void BitBoardUpdater::makeTemporaryKingMove(bool isWhite, bool isKingSide) {
    ...
    void BitBoardUpdater::unmakeTemporaryKingMove(bool isWhite, bool isKingSide) {
        U64 kingBitboard = isWhite ? _bitboards.getWhiteKingBitboard() : _bitboards.getBlackKingBitboard();
